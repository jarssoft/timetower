...
static struct argp_option options[] = {
  ...
  {"stdin",    '@', nullptr,    0, "Read XML filenames from standard input" },
  ...
  {"svg",      's', nullptr,    0, "Output in svg-format instead of html." },
  {"midnight", 'm', "MIDNIGHT", 0, "Set time when day changes in hours. Default is 5." },
  { 0 }
};
...
static error_t parse_opt (int key, char *arg, struct argp_state *state)
{
  ...
  switch (key)
    ...
    case 'w':
      ...
      break;
    case 'd':
      arguments->weekday = atoi(arg);
      break;
    case 's':
      arguments->svgoutput = true;
      break;
    case 'm':
      arguments->midnight = atoi(arg);
      break;
    case '@':
...
static void run(struct arguments* arguments){
    ...
    setWeekday(arguments->weekday);
    setMidnight(arguments->midnight);
